...
const file_sharing_js_1 = require("./file-sharing.js");
...
function getSupabaseClient() {
    ...
    return (0, supabase_js_1.createClient)(process.env.VITE_SUPABASE_URL, process.env.VITE_SUPABASE_ANON_KEY);
}
...
router.post('/create', async (req, res) => {
    try {
        ...
        const shareLink = await (0, file_sharing_js_1.createShareLink)({
            filePath,
            fileName,
            fileSize,
            createdBy: user.id,
...
router.get('/list', async (req, res) => {
    try {
        ...
        const shareLinks = await (0, file_sharing_js_1.getUserShareLinks)(user.id);
...
router.patch('/:id/deactivate', async (req, res) => {
    try {
        ...
        const { id } = req.params;
        await (0, file_sharing_js_1.deactivateShareLink)(id, user.id);
        res.json({
            success: true,
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        const { id } = req.params;
        await (0, file_sharing_js_1.deleteShareLink)(id, user.id);
        res.json({
            success: true,
...
router.get('/:token', async (req, res) => {
    try {
        ...
        const link = await (0, file_sharing_js_1.getShareLink)(token);
        if (!link) {
...
router.post('/:token/download', async (req, res) => {
    try {
        ...
        const validation = await (0, file_sharing_js_1.validateShareLink)(token, password);
        if (!validation.valid) {
        ...
        await (0, file_sharing_js_1.incrementDownloadCount)(token);
